...
// Код формы
$iAdmin_Form_Id = 307;
$sAdminFormAction = '/admin/bot/index.php';
...
$oAdmin_Form_Controller
	...
	->title(Core::_('Bot.menu'))
	->pageTitle(Core::_('Bot.menu'));

// Строка навигации
$bot_dir_id = intval(Core_Array::getGet('bot_dir_id', 0));
...
// Глобальный поиск
$additionalParams = 'bot_dir_id=' . $bot_dir_id;
...
if ($bot_dir_id)
{
	...
	$oBot_Dir = Core_Entity::factory('Bot_Dir')->find($bot_dir_id);

	if (!is_null($oBot_Dir->id))
	{
		$aBreadcrumbs = array();

		do
		{
			$additionalParams = 'bot_dir_id=' . intval($oBot_Dir->id);
		...
		} while ($oBot_Dir = $oBot_Dir->getParent());
...
{
	$oBot_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Bot_Controller_Edit', $oAdmin_Form_Action
	);

	$oBot_Controller_Edit->addEntity($oAdmin_Form_Entity_Breadcrumbs);

	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($oBot_Controller_Edit);
}
...
{
	$oAdmin_Form_Dataset
		->addCondition(array('where' => array('bot_dirs.parent_id', '=', $bot_dir_id)));
}
...
if (strlen($sGlobalSearch))
{
	$oAdmin_Form_Dataset
		->addCondition(array('open' => array()))
		->addCondition(array('where' => array('bots.id', '=', $sGlobalSearch)))
		->addCondition(array('setOr' => array()))
		->addCondition(array('where' => array('bots.name', 'LIKE', '%' . $sGlobalSearch . '%')))
		->addCondition(array('close' => array()));
}
else
{
	$oAdmin_Form_Dataset
		->addCondition(array('where' => array('bots.bot_dir_id', '=', $bot_dir_id)));
}
