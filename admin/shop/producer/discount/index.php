...
// Код формы
$iAdmin_Form_Id = 72;
$sFormAction = '/admin/shop/producer/discount/index.php';

$oAdmin_Form = Core_Entity::factory('Admin_Form', $iAdmin_Form_Id);

$oShop_Producer = Core_Entity::factory('Shop_Producer', Core_Array::getGet('shop_producer_id', 0));

$oShop_Producer_Dir = $oShop_Producer->Shop_Producer_Dir;

$oShop = $oShop_Producer->Shop;
$oShopDir = $oShop->Shop_Dir;

$sFormTitle = Core::_('Shop_Discount.show_producer_discount_title', $oShop_Producer->name, FALSE);
...
$oAdmin_Form_Entity_Breadcrumbs->add(
	...
	Admin_Form_Entity::factory('Breadcrumb')
		->name(Core::_('Shop_Producer.show_producers_title'))
		->href($oAdmin_Form_Controller->getAdminLoadHref('/admin/shop/producer/index.php', NULL, NULL, "shop_id={$oShop->id}"))
		->onclick($oAdmin_Form_Controller->getAdminLoadAjax('/admin/shop/producer/index.php', NULL, NULL, "shop_id={$oShop->id}"))
);

// Крошки по группам товаров
if ($oShop_Producer_Dir->id)
{
	$oShopProducerBreadcrumbs = $oShop_Producer_Dir;
	...
	{
		$aBreadcrumbs[] = Admin_Form_Entity::factory('Breadcrumb')
			->name($oShopProducerBreadcrumbs->name)
			->href($oAdmin_Form_Controller->getAdminLoadHref('/admin/shop/item/index.php', NULL, NULL, "shop_id={$oShop->id}&shop_group_id={$oShop_Producer_Dir->id}"))
			->onclick($oAdmin_Form_Controller->getAdminLoadAjax('/admin/shop/item/index.php', NULL, NULL, "shop_id={$oShop->id}&shop_group_id={$oShop_Producer_Dir->id}"));
	} while ($oShopProducerBreadcrumbs = $oShopProducerBreadcrumbs->getParent());
...
$oAdmin_Form_Entity_Breadcrumbs->add(
	Admin_Form_Entity::factory('Breadcrumb')
		->name($sFormTitle)
		->href($oAdmin_Form_Controller->getAdminLoadHref($oAdmin_Form_Controller->getPath(), NULL, NULL, "shop_producer_id={$oShop_Producer->id}"))
		->onclick($oAdmin_Form_Controller->getAdminLoadAjax($oAdmin_Form_Controller->getPath(), NULL, NULL, "shop_producer_id={$oShop_Producer->id}"))
);
...
{
	$Shop_Producer_Discount_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Shop_Producer_Discount_Controller_Edit', $oAdmin_Form_Action_Edit
	);
	$Shop_Producer_Discount_Controller_Edit->addEntity($oAdmin_Form_Entity_Breadcrumbs);
	$oAdmin_Form_Controller->addAction($Shop_Producer_Discount_Controller_Edit);
}
...
{
	$Shop_Producer_Discount_Controller_Delete = Admin_Form_Action_Controller::factory(
		'Shop_Producer_Discount_Controller_Delete', $oAdmin_Form_Action_MarkDeleted
	);
	$oAdmin_Form_Controller->addAction($Shop_Producer_Discount_Controller_Delete);
}
...
$oAdmin_Form_Dataset
	...
	->addCondition(
		array('join' => array('shop_producer_discounts', 'shop_discounts.id', '=', 'shop_producer_discounts.shop_discount_id'))
	)
	->addCondition(array('where' => array('shop_producer_discounts.shop_producer_id', '=', $oShop_Producer->id)))
	->addCondition(array('where' => array('shop_producer_discounts.siteuser_id', '=', 0)));

$oAdmin_Form_Controller->addDataset($oAdmin_Form_Dataset);

$oAdmin_Form_Controller->addExternalReplace('{shop_producer_dir_id}', $oShop_Producer_Dir->id ? $oShop_Producer_Dir->id : 0);
...
$oAdmin_Form_Dataset
	...
	->addCondition(
		array('join' => array('shop_producer_bonuses', 'shop_bonuses.id', '=', 'shop_producer_bonuses.shop_bonus_id'))
	)
	->addCondition(array('where' => array('shop_producer_bonuses.shop_producer_id', '=', $oShop_Producer->id)));
