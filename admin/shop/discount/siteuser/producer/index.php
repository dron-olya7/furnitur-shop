...
// Код формы
$iAdmin_Form_Id = 340;
$sFormAction = '/admin/shop/discount/siteuser/producer/index.php';
...
$oShop_Producer_Discount = Core_Entity::factory('Shop_Producer_Discount', intval(Core_Array::getGet('shop_producer_discount_id', 0)));
$oShopDir = $oShop->Shop_Dir;

$sFormTitle = Core::_('Shop_Discount_Siteuser_Producer.title', $oShop_Producer_Discount->Siteuser->login);
...
$oAdmin_Form_Entity_Breadcrumbs->add(
	Admin_Form_Entity::factory('Breadcrumb')
		->name($sFormTitle)
		->href($oAdmin_Form_Controller->getAdminLoadHref($oAdmin_Form_Controller->getPath(), NULL, NULL, "shop_producer_discount_id={$oShop_Producer_Discount->id}&siteuser_id={$oSiteuser->id}&shop_id={$oShop->id}&shop_group_id={$oShopGroup->id}"))
		->onclick($oAdmin_Form_Controller->getAdminLoadAjax($oAdmin_Form_Controller->getPath(), NULL, NULL, "shop_producer_discount_id={$oShop_Producer_Discount->id}&siteuser_id={$oSiteuser->id}&shop_id={$oShop->id}&shop_group_id={$oShopGroup->id}"))
);
...
{
	$oShop_Discount_Siteuser_Producer_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Shop_Discount_Siteuser_Producer_Controller_Edit', $oAdmin_Form_Action_Edit
	);

	$oShop_Discount_Siteuser_Producer_Controller_Edit->addEntity($oAdmin_Form_Entity_Breadcrumbs);

	$oAdmin_Form_Controller->addAction($oShop_Discount_Siteuser_Producer_Controller_Edit);
}
...
{
	$oShop_Discount_Siteuser_Producer_Controller_Delete = Admin_Form_Action_Controller::factory(
		'Shop_Discount_Siteuser_Producer_Controller_Delete', $oAdmin_Form_Action
	);

	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($oShop_Discount_Siteuser_Producer_Controller_Delete);
}
...
$oAdmin_Form_Dataset
	->addCondition(
		array('select' => array('shop_producer_discounts.*'))
	)
	->addCondition(
	...
	->addCondition(array('where' => array('shop_producer_discounts.siteuser_id', '=', $oShop_Producer_Discount->Siteuser->id)))
	->addCondition(array('where' => array('shop_producer_discounts.shop_producer_id', '>', 0)));

$oAdmin_Form_Controller->addDataset($oAdmin_Form_Dataset);

$oAdmin_Form_Controller->addExternalReplace('{shop_producer_discount_id}', $oShop_Producer_Discount->id);
