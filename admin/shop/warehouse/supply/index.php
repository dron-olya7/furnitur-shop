...
// Код формы
$iAdmin_Form_Id = 355;
$sAdminFormAction = '/admin/shop/warehouse/supply/index.php';
...
$sFormTitle = Core::_('Shop_Warehouse_Supply.title');
...
{
	$Shop_Warehouse_Supply_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Shop_Warehouse_Supply_Controller_Edit', $oAdmin_Form_Action
	);

	$Shop_Warehouse_Supply_Controller_Edit->addEntity($oAdmin_Form_Entity_Breadcrumbs);

	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($Shop_Warehouse_Supply_Controller_Edit);
}
...
{
	$oShop_Warehouse_Supply_Item_Controller_Delete = Admin_Form_Action_Controller::factory(
		'Shop_Warehouse_Supply_Item_Controller_Delete', $oAdminFormActionDeleteShopItem
	);

	$oAdmin_Form_Controller->addAction($oShop_Warehouse_Supply_Item_Controller_Delete);
}
...
{
	$Shop_Warehouse_Supply_Controller_Print = Admin_Form_Action_Controller::factory(
		'Shop_Warehouse_Supply_Controller_Print', $oAdmin_Form_Action
	);

	$Shop_Warehouse_Supply_Controller_Print
		->title(Core::_('Shop_Warehouse_Supply.title'))
		->printlayout($printlayout_id);

	// Добавляем типовой контроллер редактирования контроллеру формы
	$oAdmin_Form_Controller->addAction($Shop_Warehouse_Supply_Controller_Print);
}
...
$oAdmin_Form_Dataset = new Admin_Form_Dataset_Entity(
	Core_Entity::factory('Shop_Warehouse_Supply')
);
...
$oAdmin_Form_Dataset->addCondition(
	array('select' => array('shop_warehouse_supplies.*'))
)
->addCondition(
	array(
		'leftJoin' => array('shop_warehouses', 'shop_warehouses.id', '=', 'shop_warehouse_supplies.shop_warehouse_id')
	)
)
->addCondition(
	array(
...
if (Core::moduleIsActive('siteuser'))
{
	$oAdmin_Form_Dataset->addCondition(
		array(
			'leftJoin' => array('siteuser_company_contracts', 'siteuser_company_contracts.id', '=', 'shop_warehouse_supplies.siteuser_company_contract_id')
		)
	)
	->addCondition(
			array('leftJoin' => array('siteuser_companies', 'siteuser_companies.id', '=', 'shop_warehouse_supplies.siteuser_company_id'))
	);
}
...
if (isset($oAdmin_Form_Controller->request['admin_form_filter_2080'])
	&& $oAdmin_Form_Controller->request['admin_form_filter_2080'] != '')
{
	$oAdmin_Form_Dataset->addCondition(
...
if (isset($oAdmin_Form_Controller->request['admin_form_filter_2088'])
	&& $oAdmin_Form_Controller->request['admin_form_filter_2088'] != '')
{
	$oAdmin_Form_Dataset->addCondition(
...
if (!Core::moduleIsActive('siteuser'))
{
	$oAdmin_Form_Controller->deleteAdminFormFieldById(2088);
	$oAdmin_Form_Controller->deleteAdminFormFieldById(2080);
	$oAdmin_Form_Controller->deleteAdminFormFieldById(2087);
}
