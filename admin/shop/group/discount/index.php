...
// Код формы
$iAdmin_Form_Id = 72;
$sFormAction = '/admin/shop/group/discount/index.php';
...
{
	$oShop_Group = $oShop_Group->Shop_Group;
}

$oParentShopGroup = $oShop_Group->Shop_Group;

$oShop = $oShop_Group->Shop;
$oShopDir = $oShop->Shop_Dir;

$sFormTitle = Core::_('Shop_Discount.show_group_discount_title', $oShop_Group->name, FALSE);
...
if ($oParentShopGroup->id)
{
	$oShopGroupBreadcrumbs = $oParentShopGroup;
	...
	do
	{
		$aBreadcrumbs[] = Admin_Form_Entity::factory('Breadcrumb')
			...
			->href($oAdmin_Form_Controller->getAdminLoadHref('/admin/shop/item/index.php', NULL, NULL, "shop_id={$oShop->id}&shop_group_id={$oParentShopGroup->id}"))
			->onclick($oAdmin_Form_Controller->getAdminLoadAjax('/admin/shop/item/index.php', NULL, NULL, "shop_id={$oShop->id}&shop_group_id={$oParentShopGroup->id}"));
...
$oAdmin_Form_Entity_Breadcrumbs->add(
	Admin_Form_Entity::factory('Breadcrumb')
		->name($sFormTitle)
		->href($oAdmin_Form_Controller->getAdminLoadHref($oAdmin_Form_Controller->getPath(), NULL, NULL, "shop_group_id={$oShop_Group->id}"))
		->onclick($oAdmin_Form_Controller->getAdminLoadAjax($oAdmin_Form_Controller->getPath(), NULL, NULL, "shop_group_id={$oShop_Group->id}"))
);
...
{
	$oShop_Group_Discount_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Shop_Group_Discount_Controller_Edit', $oAdmin_Form_Action_Edit
	);

	$oShop_Group_Discount_Controller_Edit->addEntity($oAdmin_Form_Entity_Breadcrumbs);

	$oAdmin_Form_Controller->addAction($oShop_Group_Discount_Controller_Edit);
}
...
{
	$oShop_Group_Discount_Controller_Delete = Admin_Form_Action_Controller::factory(
		'Shop_Group_Discount_Controller_Delete', $oAdmin_Form_Action_MarkDeleted
	);

	$oAdmin_Form_Controller->addAction($oShop_Group_Discount_Controller_Delete);
}
...
$oAdmin_Form_Dataset
	...
	->addCondition(
		array('join' => array('shop_group_discounts', 'shop_discounts.id', '=', 'shop_group_discounts.shop_discount_id'))
	)
	->addCondition(array('where' => array('shop_group_discounts.shop_group_id', '=', $oShop_Group->id)))
	->addCondition(array('where' => array('shop_group_discounts.siteuser_id', '=', 0)));

$oAdmin_Form_Controller->addDataset($oAdmin_Form_Dataset);

$oAdmin_Form_Controller->addExternalReplace('{shop_group_id}', $oShop_Group->id ? $oShop_Group->id : 0);
...
$oAdmin_Form_Dataset
	...
	->addCondition(
		array('join' => array('shop_group_bonuses', 'shop_bonuses.id', '=', 'shop_group_bonuses.shop_bonus_id'))
	)
	->addCondition(array('where' => array('shop_group_bonuses.shop_group_id', '=', $oShop_Group->id)));
