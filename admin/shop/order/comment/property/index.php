...
// Код формы
$iAdmin_Form_Id = 67;
$sAdminFormAction = '/admin/shop/order/comment/property/index.php';
...
$oAdmin_Form_Controller
	...
	->title(Core::_('Shop_Order.show_list_of_item_comments_properties_title', $oShop->name))
	->pageTitle(Core::_('Shop_Order.show_list_of_item_comments_properties_title', $oShop->name));
...
$additionalParams = 'shop_id=' . $oShop->id . '&shop_group_id=' . $shop_group_id;
$oAdmin_Form_Entity_Breadcrumbs->add(
	Admin_Form_Entity::factory('Breadcrumb')
		->name($oShop->id
			? Core::_('Shop_Order.show_comments_title', $oShop->name, FALSE)
			: Core::_('Shop.comments_title'))
		->href($oAdmin_Form_Controller->getAdminLoadHref('/admin/shop/order/comment/index.php', NULL, NULL, $additionalParams))
		->onclick($oAdmin_Form_Controller->getAdminLoadAjax('/admin/shop/order/comment/index.php', NULL, NULL, $additionalParams))
);
...
if ($oAdmin_Form_Action && $oAdmin_Form_Controller->getAction() == 'edit')
{
	$oProperty_Controller_Edit = Admin_Form_Action_Controller::factory(
		'Shop_Order_Comment_Property_Controller_Edit', $oAdmin_Form_Action
	);
...
$oAdmin_Form_Dataset->addCondition(
	...
	array('join' => array('shop_order_comment_property_dirs', 'shop_order_comment_property_dirs.property_dir_id', '=', 'property_dirs.id'))
)/*->addCondition(
	array('where' => array('parent_id', '=', $property_dir_id))
)*/->addCondition(
	array('where' => array('shop_order_comment_property_dirs.shop_id', '=', $shop_id))
)
->changeField('name', 'type', 4)
->changeField('name', 'link', "/admin/shop/order/comment/property/index.php?shop_id=" . $shop_id . "&shop_group_id=" . $shop_group_id . "&property_dir_id={id}")
->changeField('name', 'onclick', "$.adminLoad({path: '/admin/shop/order/comment/property/index.php', additionalParams: 'shop_id=" . $shop_id . "&shop_group_id=" . $shop_group_id ."&property_dir_id={id}', windowId: '{windowId}'}); return false");
...
$oAdmin_Form_Dataset->addCondition(
	array('select' => array('properties.*', array('shop_order_comment_properties.id', 'dataTmp')))
)->addCondition(
	array('join' => array('shop_order_comment_properties', 'shop_order_comment_properties.property_id', '=', 'properties.id'))
)/*->addCondition(
	array('where' => array('property_dir_id', '=', $property_dir_id))
)*/->addCondition(
	array('where' => array('shop_order_comment_properties.shop_id', '=', $shop_id))
);
...
$oAdmin_Form_Dataset
->changeField('multiple', 'link', "/admin/shop/order/comment/property/index.php?hostcms[action]=changeMultiple&hostcms[checked][{dataset_key}][{id}]=1&shop_id=" . $shop_id . "&shop_group_id=" . $shop_group_id . "&property_dir_id={property_dir_id}")
->changeField('multiple', 'onclick', "$.adminLoad({path: '/admin/shop/order/comment/property/index.php', additionalParams: 'hostcms[checked][{dataset_key}][{id}]=1&shop_id=" . $shop_id . "&shop_group_id=" . $shop_group_id ."&property_dir_id={property_dir_id}', action: 'changeMultiple', windowId: '{windowId}'}); return false")
->changeField('indexing', 'link', "/admin/shop/order/comment/property/index.php?hostcms[action]=changeIndexing&hostcms[checked][{dataset_key}][{id}]=1&shop_id=" . $shop_id . "&shop_group_id=" . $shop_group_id . "&property_dir_id={property_dir_id}")
->changeField('indexing', 'onclick', "$.adminLoad({path: '/admin/shop/order/comment/property/index.php', additionalParams: 'hostcms[checked][{dataset_key}][{id}]=1&shop_id=" . $shop_id . "&shop_group_id=" . $shop_group_id ."&property_dir_id={property_dir_id}', action: 'changeIndexing', windowId: '{windowId}'}); return false");
