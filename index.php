...
if (Core::moduleIsActive('ipaddress'))
{
	...
	$bBlockedIp = $oIpaddress_Controller->isBlocked($aIp);
	$bBlockedUserAgent = Ipaddress_Useragent_Controller::isBlocked();

	if ($bBlockedIp || $bBlockedUserAgent)
	{
		Core_Log::instance()->clear()
			->status(Core_Log::$MESSAGE)
			->write($bBlockedIp
				? Core::_('Ipaddress.error_log_blocked_ip', implode(',', $aIp))
				: Core::_('Ipaddress.error_log_blocked_useragent', implode(',', $aIp), Core_Array::get($_SERVER, 'HTTP_USER_AGENT', '', 'str'))
			);
