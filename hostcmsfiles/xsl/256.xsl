...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/shop">
		<xsl:if test="viewed/node()">
			<div class="viewed-main">
				<div class="container">
					<div class="section-title">
						<span class="h2">Просмотренные</span>
					</div>
					<div class="row">
						<div class="col-12">
							<div class="viewed-slider js-slider-viewed products-row">
								<xsl:if test="count(viewed//shop_item) &gt; 4">
									<xsl:attribute name="class">viewed-slider js-slider-viewed products-row four</xsl:attribute>
								</xsl:if>
			...
			<div class="product-img">
				<a title="{name}" href="{url}">
					<xsl:choose>
						...
						<xsl:otherwise>
							<img class="lazyload" data-src="/hostcmsfiles/assets/images/no-image.svg" alt="{name}"/>
						</xsl:otherwise>
					</xsl:choose>
				</a>
			</div>
