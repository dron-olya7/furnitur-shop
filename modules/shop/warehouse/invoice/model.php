...
class Shop_Warehouse_Invoice_Model extends Core_Entity
{
	...
	public function delete($primaryKey = NULL)
	{
		...
		$this->Shop_Warehouse_Invoice_Items->deleteAll(FALSE);
	...
	public function getAmount()
	{
		...
		$aShop_Warehouse_Invoice_Items = $this->Shop_Warehouse_Invoice_Items->findAll(FALSE);
		foreach ($aShop_Warehouse_Invoice_Items as $oShop_Warehouse_Invoice_Item)
		{
			if ($oShop_Warehouse_Invoice_Item->shop_item_id && $oShop_Warehouse_Invoice_Item->Shop_Item->shop_currency_id)
			{
				$price = Shop_Controller::instance()->getCurrencyCoefficientInShopCurrency(Core_Entity::factory('Shop_Currency', $oShop_Warehouse_Invoice_Item->Shop_Item->shop_currency_id), $oBaseCurrency) * $oShop_Warehouse_Invoice_Item->price;

				$amount += $price * $oShop_Warehouse_Invoice_Item->count;
			}
		}
	...
	{
		return Core::_('Shop_Warehouse_Invoice.title');
	}
	...
	public function backupRevision()
	{
		if (Core::moduleIsActive('revision'))
		{
			...
			{
				$aBackup['items'][] = array(
					'shop_item_id' => $oShop_Warehouse_Invoice_Item->shop_item_id,
					'price' => $oShop_Warehouse_Invoice_Item->price,
					'count' => $oShop_Warehouse_Invoice_Item->count,
				);
			}
		...
		if (Core::moduleIsActive('revision'))
		{
			...
			if (is_array($aBackup))
			{
				...
				if (count($aAllItems))
				{
					...
					// Создаем новые
					foreach ($aAllItems as $aShop_Warehouse_Invoice_Item)
					{
						$oShop_Warehouse_Invoice_Item = Core_Entity::factory('Shop_Warehouse_Invoice_Item');
						$oShop_Warehouse_Invoice_Item->shop_warehouse_invoice_id = $this->id;
						$oShop_Warehouse_Invoice_Item->shop_item_id = Core_Array::get($aShop_Warehouse_Invoice_Item, 'shop_item_id');
						$oShop_Warehouse_Invoice_Item->count = Core_Array::get($aShop_Warehouse_Invoice_Item, 'count');
						$oShop_Warehouse_Invoice_Item->save();
					}
				}
	 ...
	 * @return array
	 * @hostcms-event shop_warehouse_invoice.onAfterGetPrintlayoutReplaces
	 */
	public function getPrintlayoutReplaces()
	{
		...
		$aReplace = array(
			...
			'shop' => $oShop,
			'user' => $this->User,
			'type' => Core::_('Shop_Warehouse_Invoice.title'),
			'total_count' => 0,
		...
		foreach ($aShop_Warehouse_Invoice_Items as $oShop_Warehouse_Invoice_Item)
		{
			...
			$aPrices = $Shop_Item_Controller->calculatePrice($oShop_Warehouse_Invoice_Item->price, $oShop_Item);

			$amount = Shop_Controller::instance()->round($oShop_Warehouse_Invoice_Item->count * $aPrices['price_tax']);
	...
	public function getDocumentFullName($oAdmin_Form_Controller)
	{
		...
		$href = $oAdmin_Form_Controller->getAdminActionLoadHref(array('path' => '/admin/shop/warehouse/invoice/index.php', 'action' => 'edit', 'operation' => 'modal', 'datasetKey' => 0, 'datasetValue' => $this->id));

		$onclick = $oAdmin_Form_Controller->getAdminActionModalLoad(array('path' => '/admin/shop/warehouse/invoice/index.php', 'action' => 'edit', 'operation' => 'modal', 'datasetKey' => 0, 'datasetValue' => $this->id, 'window' => '', 'width' => '90%'));
	 ...
	 * @hostcms-event shop_warehouse_invoice.onBeforeGetRelatedSite
	 * @hostcms-event shop_warehouse_invoice.onAfterGetRelatedSite
	 */
	...
	/**
	 * Create Shop_Warehouse_Supply_Model object
	 * @return object
	 */
	...
	public function getDocumentInfoLink($oAdmin_Form_Controller)
	{
		...
		$options = array('path' => '/admin/shop/warehouse/invoice/index.php', 'action' => 'edit', 'datasetKey' => 0, 'datasetValue' => $this->id, 'additionalParams' => "shop_id={$oShop->id}", 'window' => '');
