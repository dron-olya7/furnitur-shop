...
class Typograph_Controller
{
	...
	protected function _applyTextRules($str)
	{
		...
		/* -ка, -де, -с, -тка, -тко, например: скажите-ка, на-ка, на-кась, нате-ка, нате-кась, он-де, да-с;на-тка, на-ткась, ну-ка, ну-кась, ну-тко, гляди-тко */
		$str = preg_replace('/(?:^|([\s]))([а-я]+)[\-\s](кась|ка|де|с|-тка|-тко)([\s,\.?!]|$)/ui', '\1<nobr>\2-\3</nobr>\4', $str);
	...
	public function process($str, $bTrailingPunctuation = FALSE)
	{
		...
		*/
		...
		// Правила корректировки только снаружи тегов
		...
		if (!$preg_last_error)
		{
			...
			foreach ($aSplit as $key => $sSplit)
			{
				...
				{
					(is_null($previous) || strtolower($previous) !== '<nobr>')
