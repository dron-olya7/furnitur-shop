...
(function($){
	...
	$.extend({
		...
		showAdminFormSettings: function(admin_form_id, user_id) {
			$.ajax({
				...
				success: function(response){
					...
					$('#adminFormSettingsModal' + admin_form_id).modal('show');

					$('#adminFormSettingsModal' + admin_form_id).on('hidden.bs.modal', function () {
						$(this).remove();
					});
				}
			});
		...
		selectAdminFormSetting: function(object)
		{
			...
			if (checked)
			{
				$object.parents('.form-group').removeClass('admin-field-checked').addClass('admin-field-checked');
			}
			else
			{
				$object.parents('.form-group').removeClass('admin-field-checked');
			}
		},
				...
				success: function (result) {
					...
					if (result['result'] && result['result']['status'] && result['result']['status'] == 'ok')
					{
						var aProcessPlanInfo = result['result']['data']['process_plan'],
							oTableBody = $('#' + windowId + ' .plan-manufactures-table > tbody'),
							countManufactures;

						!oTableBody.data('aAddedProcessPlansId') && oTableBody.data('aAddedProcessPlansId', []);

						// Добавляемая техкарта отсутствует среди ранее добавленных и для техкарты задана продукция (товары)
						if (!~$.inArray(aProcessPlanInfo['id'], oTableBody.data('aAddedProcessPlansId'))
							&& (countManufactures = result['result']['data']['process_plan']['manufacture'].length))
						{
							...
							oTableBody.data('aAddedProcessPlansId').push(aProcessPlanInfo['id']);
						}
					}
		...
		deleteProductionTaskProcessPlan: function(oDelteButton) {
			...
			if (confirm(i18n.confirm_delete)) {
				...
				var oDelteButton = $(oDelteButton),
					oTr = oDelteButton.parents('tr'),
 ...
 */
		...
		applyPropertySectionSortable: function(windowId, propertyId) {
			...
			jSection.find(':input').on('touchstart', function(e){
		...
		applyInformationsystemGroupAutocomplete: function(windowId, propertyId, informationsystemId) {
			var jInput = $('#' + windowId + ' input[id ^= input_property_' + propertyId + ']'),
...
$(function(){
	...
	$('body')
				...
				&& $this.hasClass("available"))
			{
				...
				else // Редактирование сделки
				{
					...
					else
					{
						...
						currentStepLi = $("#simplewizardstep" + currentDealTemplateStepId, dealTemplateSteps);
						// currentStepName = $.escapeHtml($("span.title", currentStepLi).text());
					...
					)
					{
						...
						stepColor = $('#simplewizardstep' + dealTemplateStepId + ' span.step', dealTemplateSteps).data('color');
		...
		.on('mousedown', '.admin-table-wrap.table-draggable', function(event) {
			if (!(event.target.tagName == 'INPUT' || event.target.tagName == 'SELECT' || event.target.tagName == 'TEXTAREA' || event.target.tagName == 'SPAN'))
			{
				$(this)
					.addClass('mousedown')
					.data({
...
function calendarEventRender(event, element)
{
	...
	if (event.description)
	{
		element.find('.fc-content')
			.append('<span class="fc-description">' + $.escapeHtml(event.description).replace(/\n/g,"<br>") + '</span>');
	}
...
function fieldChecker()
{
	...
	this.restoreFieldChange = function($object) {
		...
		// console.log($object);
		// console.log($object.parents('.form-group'));
		// console.log($parentCaption);

		if ($object.getInputType() == 'select')
		{
			// console.log($object.find('option[selected]'));

			optionVal = $object.find('option[selected]').val() || 0;
			oldValue = optionVal;
